...
Elemento customizado (`<image-slot>`, definido em `image-slot.js`) que envolve as fotos de "âncora" do site — hero, "A Loja" e "Quem Somos" — com placeholder, formato (`shape`) e raio (`radius`) configuráveis por instância. Cada slot puxa a imagem do atributo `src`: hero → `photos/hero.webp`, "A Loja" → `photos/loja-1.jpg`, "Quem Somos" → `photos/quem-somos.jpg`. O sidecar `.image-slots.state.json` (que o `image-slot.js` usava pra persistir uploads feitos pelo editor) foi removido — a foto do hero, que só existia lá como data-URL base64 de 396 KB, virou o arquivo real `photos/hero.webp` (259 KB, cacheável). Se o `fetch` do sidecar falhar (não existe / bloqueado pelo `.htaccess`), o `image-slot.js` cai no `src` sem erro. A galeria (seção "GALERIA") não usa mais `<image-slot>` — virou o carrossel abaixo, com `<img>` simples (ver bug de `class` documentado em Motion, que também motivou essa escolha). O `<img>` interno do `image-slot` fica em `touch-action:auto` (não `none`) — ver Named Rules.

### Quem Somos
Seção escura (mesmo tratamento visual de "A Loja": fundo `#1E2B2A`, rótulo dourado, título e texto brancos), com a foto real dos donos atrás do balcão (`photos/quem-somos.jpg`) de um lado e a história da loja do outro. O rótulo eyebrow usa o ano de fundação (`DESDE 1995`) em vez do texto genérico das outras seções — um dado real de confiança, não decoração.

### Vídeo de Apresentação
Seção clara logo após "Quem Somos", vídeo institucional em `<video>` dentro de `ds-video-frame` (raio 20px, `max-width:960px` centralizado, `position:relative`). Toca sozinho e em loop assim que a página carrega, com controles nativos visíveis. Tenta autoplay COM som primeiro; todo navegador bloqueia isso para um visitante de primeira vez, então o `componentDidMount` cai automaticamente para autoplay mudo (sempre permitido) e revela um botão flutuante `ds-video-unmute` (canto inferior direito, ícone de alto-falante mudo + "Ativar som") que liga o som com um toque. Fonte: `videos/apresentacao.mp4`, recomprimido de 69,5MB (1080p, 12Mbps) para ~8,3MB (720p, ~1,3Mbps, H.264 CRF 24 + áudio AAC 128kbps) via `ffmpeg`, mantendo qualidade visual — o arquivo original ficou fora do projeto (pasta `Videos/`, com V maiúsculo, não rastreada).
...
- **Ícone dos cards de categoria** (`ds-cat-icon`): selo 44×44px, cantos de 10px (`rounded.sm`), fundo na cor de destaque do card a ~9% de opacidade (sufixo hex `18`) com um ícone de linha SVG (viewBox 24×24, `stroke-width:1.8`, sem preenchimento) na mesma cor a 100%, um por categoria (caneta, livro, ferramenta, carimbo, câmera, etc.) — nunca um ícone genérico repetido. Cresce sutilmente (`scale(1.08)`) no hover do card via `transform` (nunca `width`/`height`).
...
- **A tag leva `type="text/plain"`** (`<script data-dc-script type="text/plain">`). Sem isso, o navegador executa o script nativamente na hora do parse — e como `DCLogic` só existe dentro do `evalDcLogic()` do `support.js`, isso lançava um `ReferenceError: DCLogic is not defined` intermitente (corrida com o carregamento assíncrono do React/Babel via CDN: ~1 em cada 2–4 cargas frias). O `support.js` acha a tag pelo atributo `[data-dc-script]` e lê o `.textContent` independente do `type`, então `text/plain` não quebra nada e elimina a corrida. Confirmado: 4/4 cargas limpas depois da mudança.

**The No-Global-touch-action-none Rule.** `image-slot.js` estilizava seu `<img>` interno com `touch-action:none` incondicionalmente — necessário só durante o arraste de reposicionamento no editor (`.spill`, ativo apenas com `data-reframe`), mas herdado pela foto sempre visível, que não tem nenhum listener de toque próprio. Resultado ao vivo: o visitante não conseguia rolar a página com o dedo sempre que o gesto começava em cima da foto da fachada, "Quem Somos" ou loja. Corrigido removendo `touch-action:none` do `.frame img` — o arraste do editor continua funcionando normalmente porque vive inteiro em `.spill`, que manteve sua própria regra.

**The hover:hover-Guard Rule.** Handlers de `onmouseenter`/`onmouseleave` usados para pausar algo em hover (ex.: autoplay do carrossel) nunca devem confiar apenas no par de eventos em telas de toque: um toque dispara um `mouseenter` sintético mas nunca o `mouseleave` correspondente (não existe cursor para "sair"), então o estado de hover fica preso em `true` para sempre após o primeiro toque. Confirmado ao vivo: autoplay do carrossel parava de vez no mobile assim que a página era tocada. Correção: só setar o estado de hover quando `window.matchMedia('(hover: hover)').matches` for verdadeiro (dispositivo com ponteiro real) — `onmouseleave` pode ficar sem guarda, já que nunca chega a ser necessário em touch puro.
