...
Elemento customizado (`<image-slot>`, definido em `image-slot.js`) que envolve as fotos de "âncora" do site — hero, "A Loja" e "Quem Somos" — com placeholder, formato (`shape`) e raio (`radius`) configuráveis por instância, e um arquivo sidecar (`.image-slots.state.json`) que persiste qual imagem está em cada slot. A galeria (seção "GALERIA") não usa mais `<image-slot>` — virou o carrossel abaixo, com `<img>` simples (ver bug de `class` documentado em Motion, que também motivou essa escolha). O `<img>` interno do `image-slot` fica em `touch-action:auto` (não `none`) — ver Named Rules.
